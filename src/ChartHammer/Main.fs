...
module Main =
    ...
    let main (argv: string array) =
        let parser = ArgumentParser.Create<CLIArguments>(programName = "ChartHammer")
        let results = parser.Parse(argv)

        let testParse = ChartHammer.DiceParser.runTest "2d6"

        match testParse with
        | XParsec.ParseResult.Ok x -> printfn "Dice parse result: %A" x
        | XParsec.ParseResult.Error e -> printfn "Error: %A" e
